...
TAG_KEY_BACKUP_GENERATION = 'Base-Retention-days'
TAG_KEY_AUTO_BACKUP       = 'Backup-Type'
TAG_VAL_AUTO_BACKUP       = 'auto'
OFFSET_RETENTION_DAYS     = 0
...
# botocore.client.EC2 obje<...>
    ...
    print('AMI buckup task is completed({0}).'.format(ret))
...
def execute_ami_backup_task(ec2_client, ec2_resource):
    ...
    for ec2_group in response['Reservations']:
        for instance_info in ec2_group['Instances']:
            if (is_target(instance_info) == False):
                continue

            ret = create_buckup_image(ec2_client, ec2_resource, instance_info, exec_time)
            if not ret:
                print('create_buckup_image({0}) was failed.'.format(instance_info['InstanceId']))
                is_success = False
            ...
            if not ret:
                print('delete_old_image({0}) was failed.'.format(instance_info['InstanceId']))
                is_success = False
...
def create_buckup_image(ec2_client, ec2_resource, instance_info, exec_time):
    ...
    print('{0} was created.'.format(image_id))

    # 念のため、タグ設定対象のイメージ＆スナップショットが出来上がるまで待つ
    time.sleep(10)

    tags  = create_tags_to_transplant(instance_info)
...
# 関数名 ： create_tags_to_transplant
# 戻り値 ： タグ群
# 引数　 ： instance_info <dict>
# 機能　 ： バックアプ設定用のタグ群を構成する
def create_tags_to_transplant(instance_info):
    ret_tags = []
    tags = instance_info['Tags']
    for tag in tags:
...
def delete_old_image(ec2_client, ec2_resource, instance_info):
    ...
    retention_days = (int(get_tag_value(instance_info, TAG_KEY_BACKUP_GENERATION)) + int(OFFSET_RETENTION_DAYS))
    count = 0
    for img in sorted_images:
        count += 1
        if retention_days >= count:
            continue
        ...
        print('{0} was deregistered.'.format(image_id))
        ...
        for snapshot in snapshots:
            snapshot.delete()
            print('{0} was deleted.'.format(snapshot))
