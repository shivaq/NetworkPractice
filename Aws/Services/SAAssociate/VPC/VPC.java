...
インターネットにアクセス出来ないことを確認
本番では決して決して絶対にしてはいけないこと
// パブリックなサーバに pem ファイルをアップロードして、それで直接アクセス

// pem の中身をコピペして、 pem ファイルを public な方の EC2 に作成
// private な EC2 に SSH
// sudo su ⇒yum update -y ⇒セキュリティグループで HTTP を許可してるのに失敗
// なぜならば、
// ルートテーブルのターゲットに IGW がないから、インターネット接続ができない
-------------------------------------------------
